...
# `blank` is the same TITLE_W chars of nothing: the bar swaps it in while the
# title is hidden, so the slot keeps its width and the buttons do not move.
...
SRC="" OFFSET=0 SCROLL=0 TEXT="" BLANK=""
...
layout_w() {
    ...
    val=${val%%[!0-9]*}
    [ -n "$val" ] || return
    [ "$val" -eq "$TITLE_W" ] || { TITLE_W=$val; OFFSET=0; make_blank; }
}

# TITLE_W non-breaking spaces — NBSP for the same reason render() uses them:
# Pango trims trailing plain spaces, and the hidden slot would come out narrower.
make_blank() {
    printf -v BLANK '%*s' "$TITLE_W" ''
    BLANK=${BLANK// /$'\u00a0'}
}
make_blank
...
emit() {
    local js jt ja jd jb
    render
    json_escape js "$STATUS"
    json_escape jt "$TITLE"
    json_escape ja "$ARTIST"
    json_escape jd "$TEXT"
    json_escape jb "$BLANK"
    printf '{"status":"%s","title":"%s","artist":"%s","display":"%s","blank":"%s"}\n' \
        "$js" "$jt" "$ja" "$jd" "$jb"
}
